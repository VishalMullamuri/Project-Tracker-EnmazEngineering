...
const WeeklyPlanner = () => {
  ...
  const [newTask, setNewTask] = useState<PlannerForm>({
    task: "",
    employee_id: "",
    status: "Not Started",
    remarks: "",
  ...
  const weekLabel = `${formatDisplayDate(currentWeekStart)} - ${formatDisplayDate(
    weekEnd
  )}`;

  const weekStartApi = formatDateForApi(currentWeekStart);
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      ...
      const teamMembers = response.data.filter(
        (employee) => employee.role === "TEAM_MEMBER"
      );
  ...
  const fetchTasks = useCallback(async () => {
    try {
      ...
      const response = await api.get<PlannerTask[]>("/weekly-planner", {
        params: {
          week_start: weekStartApi,
        },
      });

      setTasks(response.data);
    } catch (err: any) {
      console.error("Failed to load weekly planner:", err);

      setError(
        err.response?.data?.detail || "Failed to load weekly planner."
      );
  ...
  const filteredTasks = useMemo(() => {
    return tasks.filter((task) => {
      const employeeName = task.employee_name || "";
  ...
  const handleMoveToNextWeek = async () => {
    ...
    try {
      ...
      const nextWeekStart = formatDateForApi(nextWeek);
      ...
      console.error("Failed to copy weekly planner tasks:", err);

      setError(
        err.response?.data?.detail || "Failed to copy selected tasks."
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleAddTask = async () => {
    if (!newTask.task.trim() || !newTask.employee_id) {
      setError("Task and employee are required.");
    ...
    try {
      ...
      await api.post("/weekly-planner", {
        task: newTask.task.trim(),
        employee_id: Number(newTask.employee_id),
        week_start: weekStartApi,
        status: newTask.status,
        remarks: newTask.remarks.trim() || null,
      });
      ...
      console.error("Failed to create weekly planner task:", err);
  ...
  const handleEditTask = async () => {
    ...
    try {
      ...
      await api.put(`/weekly-planner/${editingTask.id}`, {
        task: editingTask.task.trim(),
        employee_id: editingTask.employee_id,
        week_start: editingTask.week_start,
        status: editingTask.status,
        remarks: editingTask.remarks?.trim() || null,
      });
      ...
      console.error("Failed to update weekly planner task:", err);
  ...
  const handleDeleteTask = async (taskId: number) => {
    ...
    try {
      ...
      await api.delete(`/weekly-planner/${taskId}`);
      ...
      console.error("Failed to delete weekly planner task:", err);
  ...
  return (
    <Layout>
      ...
      {error && (
        <div className="mt-4 px-4 py-3 rounded-lg border border-red-200 bg-red-50 text-sm text-red-700 flex items-center justify-between gap-4">
          ...
          <table className="w-full min-w-[1100px] table-fixed">
            <thead className="bg-gray-50">
              <tr className="text-sm text-gray-600">
                ...
                <th className="py-3 px-5 text-left w-[27%] font-semibold">
                  Task
                </th>
                <th className="py-3 px-5 text-left w-[22%] font-semibold">
                  Employee
                </th>
                <th className="py-3 px-5 text-center w-[15%] font-semibold">
                  Status
                </th>
                <th className="py-3 px-5 text-left w-[24%] font-semibold">
                  Remarks
                </th>

                {isManager && (
                  <th className="py-3 px-5 text-center w-[12%] font-semibold">
                    Action
                  </th>
                )}
              </tr>
            ...
            <div className="p-5 space-y-5">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    setNewTask({ ...newTask, remarks: event.target.value })
                  }
